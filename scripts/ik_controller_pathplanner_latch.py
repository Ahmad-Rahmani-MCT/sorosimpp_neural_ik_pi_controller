    ...
    def main_loop(self, event): 
        ...
        if not self.init_pose_buf_filled: 
            ...
            if not self.latched:
                ...
                # Trigger latch ONLY if we have finished the trajectory points
                if self.counter >= self.scaled_des_trajectory.shape[0]:
                    rospy.loginfo("--- TRAJECTORY FINISHED. LATCHING AND HOLDING LAST INPUT ---")
                    self.latched = True
                    # We return here to skip the NN update. 
                    # The previous input (which was the final one) is preserved in self.current_u
                    return 
                ...
                # PHASE 2: LATCHED 
                # Do nothing. Just hold the value.
                pass
...
def main(): 
    ...
    raw_des_trajectory = lin_path_gen(-0.03, -0.06, 0, 0, 40) 

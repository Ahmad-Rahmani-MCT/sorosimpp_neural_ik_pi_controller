...
class IK_CTRL_SYS:
    ...
    def prepare_features(self, current_pose, current_input, des_ee_trajectory): 
        # This logic ensures that once counter >= len, we use the last point
    ...
    def main_loop(self, event): 
        ...
        if not self.init_pose_buf_filled: 
            ...
            if not self.latched:
                # PHASE 1: Neural Network (Always Active)
                # We removed the tolerance check (check_tolerance) and the latching logic.
                # prepare_features handles the "creeping" by clamping the target to the last point.
                ...
                # PHASE 2: Unreachable in this configuration
                # Kept only for structure compliance
                pass
